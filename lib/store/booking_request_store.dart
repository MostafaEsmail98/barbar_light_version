...
abstract class _BookingRequestStore with Store {
  ...
  double get percentTaxAmount => taxPercentage
        ...
        if (isCouponApplied && isPackagePurchase) {
          num totalAmountAfterCoupon = selectedPackageList.first.packagePrice! -
              finalDiscountCouponAmount;
          return ((totalAmountAfterCoupon * p0.percent.validate()) / 100);
        } else if (isPackagePurchase) {
          return ((selectedPackageList.first.packagePrice! *
                  p0.percent.validate()) /
              100);
        }
        ...
        if (isCouponApplied) {
          return ((calculateSelectedServiceTotalAmountWithCouponDiscount *
                  p0.percent.validate()) /
              100);
        } else {
  ...
  num get totalAmount {
    ...
    if (isPackagePurchase) {
      ...
      return selectedPackageList.first.packagePrice.validate() -
          finalDiscountCouponAmount +
          totalTax;
    }
  ...
  num get calculateTotalDiscountCouponAmount =>
      selectedServiceTotalAmount * couponDiscountPercentage / 100;

  @computed
  num get calculateSelectedServiceTotalAmountWithCouponDiscount =>
      selectedServiceTotalAmount - finalDiscountCouponAmount;
  ...
  Map<String, dynamic> toJson(
    ...
    if (paymentMethodval != null) data['payment_method'] = paymentMethodval;
    if (bookingStatus != null) data['status'] = bookingStatus;
    print('start_date_time');
    print(dateTime);
    if (dateTime != null)
      data['start_date_time'] = dateTime ?? '2025-01-25 16:12:26'; // dateTime;
    if (note.isNotEmpty) data['note'] = this.note.validate();
    if (isCouponApplied)
      data['couponDiscountamount'] = finalDiscountCouponAmount;
    ...
    if (this.isPackagePurchase && this.selectedPackageList.isNotEmpty) {
      data["packages"] =
          this.selectedPackageList.validate().map((e) => e.toJson()).toList();
      if (isCouponApplied)
        data['couponDiscountamount'] = finalDiscountCouponAmount;
      if (isCouponApplied) data['coupon_code'] = discountCouponCode;
