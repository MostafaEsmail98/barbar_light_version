...
Future<BookingStatusResponse> getInvoiceLink(
    {required String bookingId}) async {
  return BookingStatusResponse.fromJson(await handleResponse(
      await buildHttpResponse(
          "${APIEndPoints.bookingInvoiceDownload}?id=$bookingId",
          method: HttpMethodType.GET)));
}

Future<OrderStatusResponse> getOrderInvoiceLink(
...
Future<List<BookingListData>> getBookingList({
  ...
  BookingListResponse res =
    ...
    int.parse(branchId.toString()) < 0
        ? '${APIEndPoints.bookingList}?$statusData$searchBooking&per_page=$perPage&page=$page'
