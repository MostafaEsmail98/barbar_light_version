...
import 'package:frezka/screens/booking/component/your_package_component.dart';
        ...
        if (selectedPayment.id == PaymentMethods.PAYMENT_METHOD_CASH) {
          savePayment(
            ...
            Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      BookingCompleteScreen(data: bookingRequestStore),
                ));
          }).catchError((e) {
            toast(e.toString());
          });
  ...
  Future<void> savePayment(
    ...
    if (selectedPayment.id == PaymentMethods.PAYMENT_METHOD_CASH) {
      ...
      razorPayService.init(
        ...
        totalAmount:
            bookingRequestStore.calculateTotalAmountWithCouponAndTaxAndTip,
        bookingId: bookingRequestStore.bookingId.validate(),
        discountAmount: 0,
        discountPercentage: 0,
      ...
      if (!supportedCurrencies.contains(appStore.currencyCode)) {
        ...
        return;
      } else if (bookingRequestStore
              .calculateTotalAmountWithCouponAndTaxAndTip <
          100) {
        return toast(
            '${locale.totalAmountShouldBeMoreThan} ${100.toPriceFormat()}');
      } else if (bookingRequestStore
              .calculateTotalAmountWithCouponAndTaxAndTip >
          1500000) {
        return toast(
            '${locale.totalAmountShouldBeLessThan} ${1500000.toPriceFormat()}');
      }
            ...
            child: Column(
              ...
              children: [
                ...
                if (bookingRequestStore.isPackagePurchase &&
                    !bookingRequestStore.isPackageReclaim)
                  SelectPackageComponents(
                          onPackageListEmpty: _handlePackageListEmpty)
                      .paddingBottom(16),
                ...
                if (!bookingRequestStore.isPackageReclaim)
                  AnimatedListView(
                    shrinkWrap: true,
                    itemCount: bookingRequestStore.isPackageReclaim
                        ? payments.take(1).length
                        : payments.length,
                ...
                if (selectedPayment != payments.first)
                  AppTextField(
                    ...
                    onChanged: (s) {
                      ...
                      setState(() {
                        tipController.text = tipAmount > 0
                            ? "${leftCurrencyFormat()}${tipAmount.toString()}"
                            : '';
