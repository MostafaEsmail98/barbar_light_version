...
class CouponBottomSheetComponent extends StatefulWidget {
  ...
  const CouponBottomSheetComponent(
      {super.key, this.isFromBooking, this.scrollController});

  @override
  State<CouponBottomSheetComponent> createState() =>
      _CouponBottomSheetComponentState();
}

class _CouponBottomSheetComponentState
    extends State<CouponBottomSheetComponent> {
  ...
  Future<void> checkCouponData(String couponCode) async {
    ...
    await getCouponData(couponCode: Uri.encodeComponent(couponCode))
        .then((value) {
      if (value.couponListData != null) {
        if (value.couponListData!.discountType.validate() == "fixed") {
          bookingRequestStore.setFixedCouponType(true);
          bookingRequestStore.setFinalDiscountCouponAmount(
              value.couponListData!.discountAmount.validate());
        } else {
          bookingRequestStore.setFixedCouponType(false);
          bookingRequestStore.setCouponPercentage(
              value.couponListData!.discountPercentage!.toInt());
          bookingRequestStore.setFinalDiscountCouponAmount(
              bookingRequestStore.calculateTotalDiscountCouponAmount);
          if (bookingRequestStore.isPackagePurchase) {
            bookingRequestStore.setFinalDiscountCouponAmount(bookingRequestStore
                    .selectedPackageList.first.packagePrice
                    .validate() *
                    ...
                    return AnimatedListView(
                      ...
                      itemBuilder: (p0, index) {
                        ...
                        return ApplyCouponCardComponent(
                          ...
                          couponAmount: data.discountType == "fixed"
